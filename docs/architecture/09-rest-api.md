...
| `GET` | `/api/files/{file_id}/content` | Download file bytes. Requires JWT. Supports `Range` headers for partial content. Sets `Content-Disposition` for browser downloads. Full-file responses (HTTP 200) include a `Digest` trailer (RFC 9530, `sha-256`) computed as the bytes stream — clients may verify after receipt. Range responses (HTTP 206) do not include a `Digest` trailer. The control plane resolves the owning node and proxies bytes transparently — the client does not need to know which node holds the file. Records a file access via the access tracker. |
| `GET` | `/api/files/{file_id}/token` | Issue a short-lived download token for a file. Requires JWT. Returns `{ token, url, expires_in }`. The `url` field is a ready-to-use path for `GET /api/files/{file_id}/download`. Tokens expire after 2 hours and are scoped to a single file. Use this endpoint to obtain a URL that can be embedded in a `<video src>`, `<img src>`, or `<a href download>` attribute — browser-native resource loading cannot send a JWT `Authorization` header, so a self-contained signed URL is required for inline media. |
| `GET` | `/api/files/{file_id}/download?token=…` | Token-authenticated file content download. **No JWT required** — the signed token in the query string provides authorization. Accepts tokens issued by `GET /api/files/{file_id}/token`. Validates that the token is unexpired and was signed for this exact file ID. Serves the same byte stream as `/content`, including `Range` support and `Content-Disposition`. Returns `401` for missing, malformed, or expired tokens. |
