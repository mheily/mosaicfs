...
- **Download button** — fetches a signed token via `GET /api/files/{file_id}/token` and triggers a native browser download via the token URL. The browser streams bytes directly to disk; no JavaScript buffering is required, so even multi-gigabyte files respond immediately.
- **Inline preview** — the drawer fetches a token on open and uses the signed URL for all media embedding. Supported types: images in an `<img>` tag, PDFs in an `<iframe>`, video files in a native `<video controls>` player with seek support, plain text and JSON in a scrollable code block (fetched separately with a JWT `Authorization` header to obtain text content). Unsupported types show a generic file icon with the MIME type. Browser-native media elements handle progressive loading and range requests transparently via the token URL — no JavaScript involvement in the data path after the token is obtained.
